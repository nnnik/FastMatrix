/*
	This example showcases the .values property of the matrix in order
	to change values and get them
	It is a very simple example so I listed it as second
*/
...
s := "Create a random matrix`nfill it with random integers from 1 to 9:`n" . m.toString("Round")

oldValue := m.values[2, 2]	;get the value at position 2, 2
m.values[2, 2] := 42		;overwrite the value with 42
s .= "`n`nOverwrite the " . Round(oldvalue) . " in the middle with 42:`n" . m.ToString("Round")
...
s .= "`n`nReplace the first row with the first column`n multiplied by the " . Round(oldValue) . " we got before:`n" . m.toString("Round")
...
Msgbox % s . "`n`nShuffle the rows:`n" m.ToString("Round")